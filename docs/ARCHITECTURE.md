...
## 개요
기사 작성기는 **두 프로세스로 분리**된다 — Vite로 빌드하는 React SPA(클라이언트, `:5173`)와 독립 Express REST/SSE 서버(`127.0.0.1:3001`). 양쪽 모두 MVC 계층으로 구성하고, 데이터는 Node 내장 SQLite 단일 파일(`news.db`)에 저장한다. 기술 선택의 배경은 `ADR.md` 참조.
