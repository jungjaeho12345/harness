...
- 기사상태(status)는 기사 생애주기 값 RDS, DPS, RRH, RRK, DDH, DDK, DPD, EPS, EEK, EEH를 가진다 (전이 규칙은 news.md 기사 생애주기를 따른다). DPD는 DPS 기사의 삭제 승인 상태값이다(행 삭제가 아니라 상태값 전이 — DB 비파괴). EPS는 엠바고가 설정된 기사를 송고할 때의 상태(송고 대기)이고, EEK/EEH는 EPS 기사를 KILL/보류한 상태값이다. EPS는 배부 tick(외부 cron pull)이 엠바고 도래로 배부를 완결 판정하면 DPS로 전이한다(시스템 전이 — 사용자 액션이 아니며, 이력은 eventType='status', action='embargoComplete'로 남는다). tick의 개별 배부 이벤트는 ArticleHistory에 eventType='distribute', action=kind(press/nonpress)로 append-only 기록된다.
