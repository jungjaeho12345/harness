...
## 철학
외부 의존성을 최소화하고 언어/런타임 표준 기능을 우선한다(런타임 의존성은 Express와 보안 미들웨어뿐, DB·번들러·테스트는 내장/표준 도구 사용). 모든 기능은 테스트를 먼저 작성(TDD)하고 작동하는 최소 구현을 선택한다. **신뢰 경계는 서버에 둔다** — 클라이언트가 보낸 값(특히 권한)은 신뢰하지 않는다. **DB의 데이터는 절대 삭제하지 않는다**(비파괴·멱등 마이그레이션만 허용).
