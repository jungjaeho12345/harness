...
> 없다. 그래서 같은 상수가 이제 **네트워크 왕복 위에 얹힌 인위적 전역 직렬화**를 뜻한다 — 모든 요청이 커넥션
> 하나를 줄 서서 쓰고, 대기 시간에 파일 I/O가 아니라 **TCP 왕복**이 들어간다. Tomcat 기본 200 스레드가 Hikari 1을
> 기다리면 `connectionTimeout`(기본 30s)을 넘긴 요청은 예외가 되고, 이 서버에서 그것은 **500**이다. 상수를 그대로
> 둔 것은 결정이지 방치가 아니다(ADR-016 결정 6): 확대는 동시성 동작을 바꾸는 별개 결정이고, phase 74 ⑤가 세운
> 락 방어선(`controller/LogsStreamWireTest`의 「항목 22」)이 이 상수에 걸려 있다. **P3에서 다시 판단하라** —
> 판단 근거는 이 표의 「잠금·트랜잭션·풀 1」 행에 있는 테스트들이고, 늘리려면 그 테스트들이 먼저 새 상한에서
> 돌아야 한다. 이 phase는 부하 측정을 하지 않았다(§8 「다중 커넥션 동시성·락 경합」).
