...
> **줄 번호 기준(중요)**: 이 문서의 모든 줄 번호는 **기준 커밋 `b53c083`의 `docs/news.md` 본문 기준**이며, 리포 전역의 `news.md N행` 인용(코드 주석·phase 문서 등 105건)과 같은 기준이다. 이 문서를 신설하면서 news.md 1행 아래에 **안내 2줄을 추가**했으므로, **현재 파일에서 실제 위치는 `N+2`행**이다(스윕과 인용 검증은 안내 추가 이전 원문에 대해 수행했다). 각 항목은 **스펙 원문을 그대로 인용**하므로 번호가 흔들려도 문장으로 대조할 수 있다.
...
1. **스펙을 바꾸지 않는다.** `docs/news.md` 본문은 한 글자도 수정하지 않았다(1행 아래에 이 문서를 가리키는 안내 2줄 = 인용 1줄 + 빈 줄만 추가했다 — `git diff --stat`은 `2 insertions(+)` 뿐이다). 원문 보존이 이 리포의 규율이다. 여기 적힌 판정은 원문을 **덮어쓰는 읽기 규칙**이지 원문의 개정이 아니다.
...
- **news.md 줄 번호 대조**: 44행 전건을 `sed`로 원문 확인(기준 = 안내 2줄 추가 **이전**의 `b53c083` 원문 — §1의 줄 번호 기준 참조). **41건 일치 · 3건 정정** — L226→**225**(작성자) · L238→**239**(잠금 API) · L252→**254**(ContentsVO). 추가로 절 표제를 포함하던 범위 1건을 조정했다(L227-233 → **228-233**).
