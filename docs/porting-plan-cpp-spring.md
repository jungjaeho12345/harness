...
7. `ContentsVO.md` — news.md가 참조하나 리포에 부재. 포팅 전 원본 확보 필요.
   - **[해소 2026-09-10]** 확인 결과 이 파일은 리포에 **체크인된 적이 없다**(전역 `find -iname "ContentsVO*"` 0건). 그 명세를 대신하는 것은 셋이다: ① 필드 정의 = **`docs/SCHEMA.md`의 `## Contents Table` 절(L42-53)** — 그 절이 스스로 "ContentsVO에 대한 명세서"(L43)라고 밝힌다. ② 편집 페이지 매핑 규칙 = **`docs/news.md:254-255`**(무엇이 입력란에 채워지고 무엇이 읽기전용으로 표시되는지 인라인 열거). ③ 응답 투영(무엇이 실제 와이어에 나가는가) = **동결 계약**(`docs/api-contract/endpoints.json` 39 + `contract/**`) — 잠금 컬럼 `lockerSessionId`·`lockerClientId`는 응답에서 제거된다. **원본 문서가 나타나면 이 셋과 대조하라**(어긋나는 항목은 새 오버라이드 후보다). 즉 '원본 확보'는 P4 착수의 차단 조건이 아니라 **대조 대상의 획득**이다. 상세: **`docs/news-md-overrides.md` §5**.
8. news.md 드리프트 2건 정리 — 301행 CORS 서술(현행은 ALLOWED_ORIGINS 체계), 174행 spellcheck=true(ADR-011이 셸 한정 무효화). **포팅 요구사항 정본으로 쓰기 전에 "ADR이 news.md를 덮어쓴 지점" 목록을 확정**해야 한다.
   - **[해소 2026-09-10]** 목록을 확정해 **`docs/news-md-overrides.md`** 로 분리했다(기준 커밋 `b53c083` · news.md 302행 전수 · 6구간 병렬 대조 + 2중 검증 78표 중 77 유지 · 인용 전건 재확인). **결과: 드리프트 39건**(override 9 · partial 16 · silent-gap 14) **+ no-conflict 확인 5건.** 즉 **여기 씨앗으로 적어 둔 2건(301행 CORS · 174행 spellcheck)은 전수 39건 중 2건이었다.** 심각도 high는 13건이고 그 축은 서버·저장소(Spring/MySQL) · 인증(bcrypt+계정 잠금 · 사용자 삭제 없음 · 세션 신원 매 요청 재도출) · 동시성(편집 잠금 신원 = 세션이 아니라 **탭 `x-edit-client`** · 잠금 컬럼 응답 비노출) · SSE(단일 무효화 신호 + 전체 재조회) · SQLite SP 부재(앱 계층 구현) · UI(툴바 15버튼이 실제로는 2개) · CORS 허용목록이다. P4~P7은 **news.md와 이 문서를 함께** 읽는다 — news.md 줄 번호를 만나면 그 문서의 표가 이긴다.
