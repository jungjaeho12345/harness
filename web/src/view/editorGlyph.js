// 약물(glyph) 삽입 순수 계산 — 약물(짧은 문자열)을 캐럿 {lineIndex, offset} 위치의 텍스트 줄 안 컬럼에 삽입한다.
// 좌표는 blocksToText(blocks)(텍스트 블록만 개행으로 이은 평문)의 절대 오프셋 기준이다(editorFind와 동일 좌표계).
// 순수 함수, DOM/window/transport/React 비의존. 입력 blocks는 변형하지 않는다(새 배열/새 textBlock만).
// 임베드·"(끝)"·블록 순서·개수는 불변 — 약물은 텍스트 블록의 text만 바꾼다.
// UI/결선/DOM 반영(updateField('body', serialize(...)) + setPendingCaretLine)은 Step 2/4 WriterPage가 한다.
// editorShortcuts.js / editorFind.js 패턴을 그대로 따른다.
...
// 약물 문자열 정규화(트림). 빈값/null/undefined → '' (UI에서 빈 약물 버튼/입력을 no-op 판정하는 데 재사용 가능).
export function normalizeGlyph(glyph) {
  if (glyph === null || glyph === undefined) return '';
  return String(glyph).trim();
}

// 텍스트 블록(blocks 배열)에서 "(끝)" 마커가 아닌 마지막 텍스트 블록의 배열 인덱스를 구한다.
// 폴백 삽입 대상 — "(끝)" 텍스트는 건드리지 않는다(phase14 replaceAll과 정합). 없으면 -1.
...
// 약물(glyph)을 캐럿 {lineIndex, offset}의 텍스트 줄 안 컬럼에 삽입한 새 블록 배열을 돌려준다.
// 반환: { blocks, caretTextLine }
//   - blocks: 삽입 결과 블록 배열(입력 mutate 금지 — 새 배열).
//   - caretTextLine: 삽입된 줄의 텍스트-줄 인덱스(number) 또는 null(삽입 실패/no-op).
//
// 규칙:
//  1) glyph 빈값/공백뿐 → no-op: { blocks: normalizeBlocks(blocks), caretTextLine: null }.
...
export function insertGlyphAtCaret(blocks, caret, glyph) {
  ...
  if (g === '') return { blocks: list, caretTextLine: null };
  ...
  if (blockIndex < 0) {
    ...
    if (fallbackIdx < 0) {
      // 텍스트 블록이 전혀 없음(임베드만/빈 배열) — 첫 텍스트 블록을 만들어 약물만 담는다(임베드 보존, 맨 앞).
      const next = [textBlock(g), ...list];
  ...
  next[blockIndex] = textBlock(old.slice(0, col) + g + old.slice(col));
