...
## 운영 — 배부 tick (엠바고 시점 배부)

- 앱에는 스케줄러가 없다(ADR-008). 외부 운영 cron이 주기적으로 `POST /api/distribution/tick`을 **Z 계정 세션**으로 호출하면, 그 시점에 도래한 엠바고 기사(EPS)가 배부되고 완결 시 DPS로 전이한다. 요청 바디는 사용하지 않는다(대상/시각/actor 주입 차단 — 신뢰 경계는 세션).
- 배부 스풀 미설정(`DIST_SPOOL_DIR` 없음) 시 이 엔드포인트는 **503**(`spool-disabled`)을 반환한다. 이전 tick이 실행 중이면 **409**(`tick-in-progress`).
- **엠바고 시각 입력 형식 주의**: `embargoAt`/`secondEmbargoAt`는 **타임존 오프셋을 포함한 ISO-8601**(`2026-07-30T09:00:00+09:00` 또는 `...Z`)이어야 tick이 도래로 인정한다. 오프셋이 없거나 파싱되지 않는 값은 배부되지 않고 기사가 EPS에 머문다(서버 TZ에 따른 조기 반출을 막는 fail-safe).
