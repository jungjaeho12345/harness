...
description: 코드 변경사항을 실제로 실행해 검증하는 테스트 하네스 스킬이다. 테스트·lint·typecheck·build 같은 검증 명령을 직접 돌리고, 실패 로그를 재현·분석하며, 회귀 테스트를 작성하고, 실행으로 확인한 사실과 추정을 구분해 보고한다. 사용자가 "테스트 돌려줘", "테스트 통과하는지 확인해줘", "왜 실패하는지 분석해줘", "빌드/타입체크 깨졌는지 봐줘", "CI 빨간불 재현해줘", "변경 범위에 맞는 테스트만 실행", "회귀 테스트 추가", "커버리지 확인" 같은, 명령을 실제로 실행하는 검증을 요청할 때 사용한다. 반대로 코드를 읽고 정적으로 버그·보안·설계 리스크를 판단하거나 severity가 있는 리뷰 코멘트를 작성하는 일은 claude-code-review-harness 스킬을 쓴다. 이 스킬은 검증 명령 실행과 테스트 결과 해석에 집중한다.
---
