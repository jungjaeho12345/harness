...
## BLOCKED (step0 판정 · 2026-09-05)

**status = `blocked`.** 이 step은 착수 전에 사람의 답 한 줄이 필요하다.

**막힌 조건 — 정확히 무엇이 필요한가**

- `docs/cutover-p3.md` §0-5묶음의 첫 행(**운영 서버의 `RCV_SPOOL_DIR` 설정 여부와 값**)이 `미상`이다.
  같은 문서 §0-2의 **Q1**이 그 질문이다: 「운영 서버가 FTP 스풀 수집을 실제로 쓰고 있습니까 — bat/서비스에 그 값이
  설정돼 있고 외부 FTPd가 그 폴더에 파일을 떨어뜨립니까?」
- step0이 확인할 수 있었던 것은 **대조군뿐**이다: 배포 템플릿 `packaging/server/기사작성기-server.bat` **41행이 주석
  처리 상태**이고, 개발 빌드 산출물 `dist/기사작성기-server/`의 bat은 **그 템플릿과 바이트 동일**이며 그 폴더의
  `data/rcv-spool`은 **비어 있다**. 이것은 **개발 머신의 사실**이지 운영의 사실이 아니다(이 머신에는 3001 리스너도
  tick 작업도 없다 — 운영기가 아니다). **추측으로 채우지 않는다.**

**답에 따라 무엇이 갈리는가**

| 답 | 이 step의 위치 | 런북에서의 순서 |
|---|---|---|
| **쓰고 있다** | **컷오버의 필수 선행** — 스위퍼가 없으면 Node를 내리는 순간 수집 경로가 사라진다 | Spring 기동과 **같은 정지 창 안에서** 스위퍼 등록까지 끝낸다 |
| **안 쓴다** | **Node 은퇴의 전제**로 내려간다. 도구는 그대로 만들되(만들지 않으면 은퇴 시점에 이 축이 다시 공백으로 튀어나온다) 검증 강도와 런북 위치가 달라진다 | 컷오버 창 밖 · 「Node 은퇴 전제 조건」 목록으로 |

**막지 않는 것**: step1은 이 답 없이도 진행한다 — 계획이 그렇게 적었다(step1.md 34행·92행: 「미상이면 결정 4를
**조건절로만** 쓴다」). step2~step5도 리포 안 자산만으로 완결되므로 영향이 없다.
