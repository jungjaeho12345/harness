...
- `phases/71-spring-distribution/index.json` — decisions **(8)(9)(15)(16)**
...
- `server-spring/src/main/java/harness/news/service/ArticleLifecycleService.java` 59행·211~230행 — phase 69의 `EMBARGO_COLUMNS`·`embargoAware`·`embargoSet`. **이 step은 그 코드를 고치지 않는다**(decisions (16))
...
그 밖(오프셋 없는 날짜-시각 `2026-01-01T09:00`, 레거시 문자열, 빈 문자열, 비문자열)은 **null**이다. 근거와 divergence는 decisions (9)에 있다 — **틀리는 방향이 안전측**(미도래 → 배부 안 함 → tick `invalid`에 표면화)이라는 사실을 테스트로 못 박는다.
...
11. **`ArticleLifecycleService.embargoSet`와의 정합성**(decisions (16)): `embargoAt`/`secondEmbargoAt` 2컬럼의 모든 조합(빈 문자열·null·값 있음 4×4)에서 `embargoSet == (requiredKinds().size() > 0)`임을 단언한다. **두 코드를 합치지는 않는다** — 드리프트만 잠근다.
...
- 3번 증분 = `.../service/EmbargoPolicy.java` · `.../service/NodeInstants.java` · 대응 테스트 2개 · `phases/71-spring-distribution/index.json`.
...
7. **변이 (f) 원복**: `ArticleLifecycleService.embargoSet`를 `embargoAt`만 보도록 좁혀 11번 red 확인 → 원복.
8. AC 실행. index.json step7 상태 갱신.
...
- `ArticleLifecycleService`의 `embargoAware`/`embargoSet`을 리팩터링하지 마라. 이유: Node도 그 자리를 인라인으로 둔다 — 합치면 계약이 잠근 DES 진입 판정과 완결 요건 판정이 한 함수에 묶인다(decisions (16)).
