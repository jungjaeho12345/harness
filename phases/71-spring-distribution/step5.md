...
- `phases/71-spring-distribution/index.json` — decisions **(4)(5)(19)(21)(22)** · order (b)① · open_questions (c)
...
- `application.properties`에 `app.host=${HOST:127.0.0.1}` · `app.collection-token=${COLLECTION_TOKEN:}` 추가(주석: 값은 OS 환경변수에서 온다 · 빈 값 = 미설정).
- `AppProperties`에 두 필드 추가. `host`는 빈 값이면 `127.0.0.1`로 수렴(Node `resolveHost`의 "빈 문자열/공백은 기본값" 규율). `collectionToken`은 **trim하지 않는다**(토큰 값을 서버가 고치면 클라이언트와 갈린다) — 다만 **빈 문자열·공백만 있는 값은 '미설정'**으로 본다(Node의 truthy 판정: `''`는 falsy, `' '`는 truthy다 → **공백 1칸은 '설정됨'이다**. Node 그대로 가라: `!process.env.COLLECTION_TOKEN`은 `' '`에 대해 false다).
- **`CollectionTokenSource`** — 1메서드 seam(`String current()`), 기본 구현이 `AppProperties`에서 읽는다(decisions (4)). 컨트롤러는 이 seam만 본다.
...
- 응답은 전부 `JsonHttp` 한 지점으로만 쓴다(decisions (22)).
...
- `scripts/spring-contract.mjs` SCOPE:
  ...
  - `failclosed.files`를 `['contract/cases/failclosed/collection-disabled.contract.js']`로 채운다.
...
- 5번 증분 = `.../service/CollectionAccess.java` · `.../service/CollectionTokenSource.java` · `.../controller/CollectionController.java` · `.../config/AppProperties.java` · `src/main/resources/application.properties` · `.../web/ReasonStatus.java`(1행) · 테스트 2개 + `HandlerInventoryTest.java` · `scripts/spring-contract.mjs` · `phases/71-spring-distribution/index.json`.

## 검증 절차

1. **red 먼저**: 와이어 테스트를 구현 전에 돌려 실패 서명을 실측한다(현재는 404가 나온다 — 그 사실을 기록).
2. **진단 실행**: `node scripts/spring-contract.mjs --profile failclosed`를 구현 전에 돌려 실패 지점을 확인하고, 구현 후 green으로 바뀌는지 대조한다.
...
7. AC 실행. 리포 `news.db`·`uploads/` 무변 · java 프로세스 잔존 0 · 리포트·로그에 토큰 값 0건.
8. index.json step5 상태 갱신.
...
- scope 표에 배부 계약 파일(`distribution-tick`·`distribution-disabled`)을 미리 넣지 마라. 이유: 확정 red이며, scope는 green이 되는 step(step15)에서만 늘린다.
