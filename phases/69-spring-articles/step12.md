...
5. **투영 누출 최종 확인**: 정적 스캔 테스트 green + 계약 리포트·stdout 어디에도 잠금 세션/탭 컬럼명이 없음을 확인한다.
6. 범위 정합(D) 결과를 요약에 붙인다.
7. index.json step12 status·summary 갱신 + `phases/index.json`의 69 항목 note를 마감 형식(`[완료 YYYY-MM-DD: ...]` + 기존 설명)으로 갱신한다.
