...
### C. 응답 투영 (타입 경계의 출발점 — decisions (4)①)

- **원본 행 명목 타입**을 여기서 정의한다: `Contents` 행은 컬럼 순서 맵을 감싼 **불투명 타입**(`ContentsRow` 등가)이다. **공개 getter·직렬화 프로퍼티를 두지 마라** — 컬럼 접근은 조회 메서드 1개로 충분하다. 감싸는 것은 타입뿐이고 키 집합·순서·NULL 키 보존은 decisions (5) 그대로다.
- 투영은 **`원본 행 → 제거 2컬럼을 뺀 새 맵(27키)`**이다(원본 mutate 금지, 키 순서 유지). **응답으로 나갈 수 있는 형태는 이 출력뿐**이고 원본 타입은 직렬화 대상이 아니다. 제거 목록은 **상수 1곳**이 소유한다.
- 배열/컬렉션 입력 방어(원소별 투영)도 정본과 같이 둔다.
- 이 타입 경계를 기계로 잠그는 테스트는 step11이 만든다(컨트롤러 패키지에 원본 타입 부재 + 직렬화 안전망). 여기서는 **타입이 갈라져 있다는 사실**만 만든다.
...
1. 전이표 **전수**: **표의 서로 다른 허용 칸 15개**(계약이 도달하는 13 + `EPS`발 2 — excluded (d))를 전부 덮고, 그것을 **계약 케이스 16건**과 대조한다(케이스 수와 칸 수가 다르다 — 같은 칸을 두 번 관측하는 케이스가 있다. '16칸'이라고 쓰면 표를 한 칸 더 만들게 된다). 그리고 대표 거부 칸(`DPS`+R+send · `RDS`+R+approveDelete · `DDH`+D+hold · `RRK`+D+send · `DPD`+D+send) → 사유 토큰까지 단언.
...
6. 투영: 2컬럼 제거 · 다른 잠금 3컬럼 유지 · 원본 불변 · 값이 null인 키 **유지** · 입력이 **원본 명목 타입**이고 출력이 **맵**이다(타입이 갈라져 있다는 것을 시그니처로 단언 — decisions (4)①).
