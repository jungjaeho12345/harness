...
- **`forward_notes`를 추가**한다(배열). 최소 항목:
  ...
  4-2. **소유자 없는 이월 1건(반드시 적어라 — step5가 넘긴 의무)**: **서버 주소를 저장하고 나면 앱 안에서 설정 화면으로 돌아갈 방법이 없다.** 설정 화면은 부팅 시 `serverUrl`이 없을 때만 뜨고(step5), 주소 변경 진입점이 될 메뉴가 P4에 없다(메뉴는 P7 범위). **P4의 어느 step도 이 경로를 소유하지 않는다** — 현재 우회는 `%APPDATA%\기사작성기-qt\config.json`을 지우거나 고치는 것뿐이다(Electron 셸의 대응 경로와 비교해 적어라). 다음 소유 후보(P7 메뉴 또는 P8 통합)와 그 전까지의 운영 안내를 함께 남긴다.
