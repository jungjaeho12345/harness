...
- `news.db` md5 무변.
- **런북 명령별 실행 결과표가 summary에 기록됐다 — 미기록 시 이 step은 미완이다.** 다른 step의 「변이 결과표」와 동형의 종료조건이다: 이 step은 코드가 아니라 **절차**를 산출하므로, 그 절차가 공허하지 않다는 증거는 **각 명령을 실제로 실행한 결과**뿐이다. 표의 열은 **① 명령(런북의 몇 번 항목) ② 실행 여부 ③ 종료코드/관측값 ④ 실패 시 무엇이 일어났는지**이고, 실행하지 않은 항목(운영 컷오버 3·4·5·6 등)은 **「미실행 — 이유」** 로 명시한다. 롤백 절차(7)는 **반드시 실행 열이 채워져야 한다**(리허설 1회).
