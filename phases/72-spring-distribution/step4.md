...
- `phases/72-spring-distribution/index.json` — decisions **(6)(15)(16)(19)(25)** · excluded **(l)(m)**
...
- 이 phase의 step1·step2·step3 산출물(`EmbargoPolicy`·`DistributionFailureLog`·`SpoolWriter`)
...
- **그 조회의 상한은 표시용 창이 아니다(decisions (16)).** Node는 `distributionService.js` 30행의 `FAILURE_DEDUP_SCAN_LIMIT`(= 사실상 무제한)을 쓴다 — **값은 그 소스에서 실측하라.** 표시용 목록 창(`distributionRetryService.js`의 기본/최대치)을 여기에 쓰면 창 밖으로 밀린 최신 실패를 못 보고 억제가 무의미해진다(같은 실패가 tick마다 새 행으로 무한 누적된다). 조회가 `articleId` 스코프로 좁혀져 있어 비용은 작다 — **세 상한을 하나로 합치지 마라.**
...
- **`Failed`는 내부 타입이고 `spoolDir`을 담는다** — 그 값이 HTTP로 나가지 않게 하는 것은 tick의 투영 책임(step7)이다. 이 사실을 javadoc에 명시한다.
- 통지 seam은 phase 68~70 관례(`HistoryErrorListener`/`HistoryErrorLogger`)를 따른다. 통지 payload는 **식별자와 고정 사유만**(경로·본문 금지).
- **트랜잭션 경계(decisions (19))**: 이력 insert + `distributedAt` update처럼 여러 문장이 묶여야 하는 자리만 `TransactionTemplate`으로 감싼다. **한 배부 호출 전체를 하나의 트랜잭션으로 묶지 마라** — 수신처 A 성공 후 B 실패에서 A의 이력까지 롤백되면 '나간 파일은 있는데 기록은 없는' 상태가 되어 다음 tick이 중복 배부한다(회수 불가).
...
15. **동시 삽입 id 귀속 — 회귀 잠금**(decisions (19)): 여러 스레드가 동시에 실패 이력을 넣어도 각자 자기 id를 받는지. **지금은 안전하다**(`ArticleHistoryRepository.insert`가 `GeneratedKeyHolder`로 삽입과 id 회수를 한 문장에서 처리한다 — 111~119행). phase 70에서 실측 재현된 id 오배정은 `last_insert_rowid()`를 **별도 문장**으로 부르던 다른 리포지토리의 문제였다. 이 테스트는 **그 안전이 유지된다는 사실을 지키는 회귀 잠금**이다(이력 id가 재전송 식별자이므로 뒤바뀌면 남의 실패를 재전송한다).
16. **중복 억제 조회 상한**: 표시용 창보다 훨씬 많은 실패 행을 쌓은 뒤에도 최신 실패가 억제 판정에 보이는지(상한을 표시용 창으로 좁히면 red — decisions (16)).
...
- 3번 증분 = `.../service/DistributionService.java` · 대응 테스트 · `phases/72-spring-distribution/index.json`.
...
7-b. **변이 (g) 원복**: 중복 억제 조회 상한을 표시용 창 값으로 좁혀 16번 red 확인 → 원복.
8. AC 실행. 리포 안에 스풀 파일 0(모든 테스트가 `@TempDir`).
9. index.json step4 상태 갱신.

## 금지사항

- 상태 전이(`status` 쓰기)를 여기서 하지 마라. 이유: 생애주기는 `ArticleEmbargoService`/`ArticleLifecycleService` 단일 출처다(step5) — 갈리면 DES/EPS/DPS 판정이 발산한다. 이 서비스는 status를 **읽기만** 한다.
- `EMBARGO_DISTRIBUTABLE_STATUSES`를 복제하지 마라. 이유: `EmbargoPolicy`가 단일 출처다.
- 병렬 배부·비동기 실행·재시도를 넣지 마라. 이유: ADR-008 (6) + 결정성 + phase 71-spring-collection step1 게이트.
