...
6. **divergence 전수(고치지 않고 기록)** — ① `path.extname`을 win32 알고리즘으로 구현. **[2026-08-29 ⑤ 반영 정정]** 이 괄호의 원래 문구("POSIX 호스트에서 갈리는 입력이 있으나 **화이트리스트 판정 결과는 같다**")는 **거짓**이다 — 반례가 실재한다: `a.png\`는 win32에서 `.png`를 뽑아 **200**이고 posix에서는 `.png\`라 **400**이며, `C:.png`는 반대로 win32 400 / posix 200이다(`server-spring/README.md`가 이미 이 반례로 기록을 정정해 두었다). 배포 호스트가 win32라 런타임 영향은 0이지만 "결과는 같다"고 적으면 안 된다. `UploadNames.java`의 구현 자체는 Node `lib/path.js`의 win32 `extname`과 **문자 단위로 일치**함이 확인됐다(고치지 않는다 — 고칠 것은 서술뿐이다). ② `Photo` 조회가 `SELECT *`가 아니라 6컬럼 명시(컬럼 추가 시 안전측) ③ 정적 서빙의 ETag/`Cache-Control`/`Accept-Ranges` 헤더가 express.static과 다르다(계약 리포트가 싣지 않는 헤더). **[2026-08-29 ⑤ 반영 정정]** 이 항목과 함께 동결했던 "Win32 이름 별칭 200 6종은 정본도 같은 파일시스템 위에서 도므로 divergence가 아니다"는 **거짓**이었다 — 정본은 `fs.stat`(libuv)이고 libuv는 `\\?\` 장문 경로라 Win32 이름 정규화를 **받지 않는다**. 같은 호스트 실측: `.png.`·`.png%20`·`.png%2e`·`.png..`·`.png%20%20`·`.png%20.`·`<하위디렉토리>./<파일>`은 정본 **404**인데 이 서버는 **200**이었고, `CON`·`NUL`·`AUX`는 정본 404인데 이 서버는 **500**이었다(장치가 실재해 조회가 예외를 던진다). 리소스 핸들러 층의 `UploadsResourceResolver`로 **닫았다**(기록만 하고 넘기지 않았다). 정본도 200인 `::$DATA`(`application/octet-stream`)·`.PNG`·`./<hex>.png` 셋은 divergence가 아니므로 그대로 둔다. ④ 요청 본문 크기 상한. **[2026-08-29 ⑤ 반영 정정]** "Spring에는 없다"는 이제 **`POST /api/upload`에는 있다**로 정정한다(10 MiB · 디코드 전 원문 바이트 · `Content-Length` 불신 · 경계값 통과 · 초과 500 `internal-error` — 전부 정본 실측에 맞췄다). 그 라우트만 막은 이유는 base64 본문이 **큰 본문을 정상적으로 받는 유일한 라우트**라 상한 부재가 곧 힙 소진 경로였기 때문이다. **전역 100kb와 기사 쓰기 라우트 10mb는 여전히 없다**(전역 도입은 37 라우트 전부의 거부 경계를 한꺼번에 움직이는데 그 경계를 관측하는 계약이 하나도 없다). 초과 시 **응답 시점**은 갈린다(정본은 요청이 끝날 때까지 응답하지 않는다 — `body-parser`의 `onFinished`. 이쪽은 즉시 500). ⑤ 비-png 확장자의 `Content-Type`(step8 관측 기록) ⑥ open_questions (1)의 결정(**이식한다 — ② 확정**)과 그 대가(예외 파일 +1 · 키 설정 경로는 계약이 동결하지 못한다) ⑦ **[② 검토 반영 · 문서 부채] Node 주석의 ADR 오인용** — `src/services/mediaSearch.js` 1행과 `src/services/translate.js` 2행이 서버 프록시를 'ADR-005'로 인용하는데 **ADR-005는 SSE 단방향 무효화 스트림 결정**이다. 실제 근거는 이 phase가 step2에서 신설한 **`ADR-014`**다. `src/**`는 무수정 목록이므로 **고치지 않고 기록만 한다** — Node 주석을 손대는 phase(P2/P3 전환 소유)가 함께 정정하라. ⑧ **`HttpRequest.timeout`의 잔여 위험**(응답 헤더까지만 덮는다 — 본문을 천천히 흘리는 외부 API는 Tomcat 워커를 점유한다. 71a가 실측한 것과 동일하며 이제 그 표면이 미디어·번역까지 넓어졌다).
