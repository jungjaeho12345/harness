...
3. `SseHttp.Stream s = sse.open(request, response)` — 헤더 3종 + 200 커밋 + flush.
4. `s.write(SseHttp.READY)`.
5. `AutoCloseable sub = changeBus.subscribe(kind -> { ... })`; 콜백 내용:
...
6. `seal()` = ① `sub.close()`(구독 해제) → ② `s.write(SseHttp.UNAUTHORIZED)` → ③ `s.close()`. **멱등**이어야 한다.
7. `s.onClosed(() -> sub.close())` — 클라 끊김·컨테이너 종료 시 구독 해제(누수 0).
...
- **구독 등록은 ready write 뒤**에 한다(Node와 같은 순서). 앞에 두면 ready 이전에 change가 끼어들 수 있다.
...
11. **구독 누수 0**: 스트림 N개(예: 5개)를 열고 전부 닫은 뒤 `ChangeBus.subscriberCount()`가 **0**으로 돌아온다. 클라이언트가 소켓을 그냥 끊은 경우(정상 종료 프레임 없이)에도 0으로 돌아온다.
12. **워커 점유 0**: 스트림을 열어 둔 채 **다른 라우트 요청 N건**(N > Tomcat 기본 워커 수보다 작아도 좋다 — 최소 10건)이 정상 응답한다. 블로킹 구현이면 여기서 막힌다.
13. **구독 콜백이 응답을 뒤집지 않는다**: 구독자 write가 실패하는 상태에서 트리거 요청이 여전히 200이다.
14. **응답 위생**: 200 응답 전문·헤더에 **64-hex 세션 토큰 0건** · 드라이브 문자로 시작하는 절대경로 0건.
15. `HandlerInventoryTest` 38행 green.
...
| M4-11 | 구독 등록을 ready write **앞**으로 이동 | Java 순서 단언 red(있다면) · 없으면 그 사실을 공백으로 기록하고 항목을 추가하라 |
