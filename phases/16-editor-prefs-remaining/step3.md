...
**범위는 등록/목록/삭제 UI + localStorage 영속까지다.** 약물의 실제 **에디터 입력 동작·약물바(glyph bar)·키조합 인터셉트는 이 step에서 하지 않는다**(aux-tools 후속 phase 소관 — 아래 DEFERRED EFFECTS). WriterPage에 약물바 placeholder 토글(`showGlyphBar` 등)이 있더라도 그 바는 미렌더 상태이며 이 step과 무관하다(건드리지 마라).
...
- `phases/16-editor-prefs-remaining/step0.md` — `glyphFavorites`/`glyphKeymap` 키 구조·기본값(`items: []`)의 **단일 출처**.
...
- `web/src/view/EditorPrefsDialog.jsx` — **변경 대상. 반드시 정독하라.** step1/step2가 추가한 탭들(편집/맞춤법)이 이미 있다(같은 phase, 순차 실행). `PREF_TABS`, form state 패턴, `open` 재초기화, `apply()` 합성 체인(colors·autosave·byline·edit·spellcheck), `reset()`, 탭 렌더 블록, testid 컨벤션을 미러링.
...
`{ key: 'glyphFavorites', label: '자주쓰는 약물' }`, `{ key: 'glyphKeymap', label: '사용자 키보드 약물' }`를 약물군 위치(맞춤법 근처)에 추가한다. 기존 탭 순서 동작은 깨지 않는다.

### 2. form state + open 재초기화 + reset (3-지점 동기화) + 등록 입력 버퍼 state
...
- 등록 입력칸용 별도 로컬 state(미커밋 입력 버퍼): `favInput`(string), `keyInputKeys`/`keyInputGlyph`(string).
...
- 키보드 약물 등록: keys·glyph **둘 다** 비어있지 않을 때만 `setGlyphKey((g) => ({ ...g, items: [...g.items, { keys: keyInputKeys.trim(), glyph: keyInputGlyph.trim() }] }))`. 둘 중 하나라도 비면 no-op.
...
기존 `setEditorPref` 체인에 두 카테고리를 끼운다 — `setEditorPref(prev, 'glyphFavorites', { items: glyphFav.items })`, `setEditorPref(prev, 'glyphKeymap', { items: glyphKey.items })`. **items 배열 전체를 넘긴다.** colors·autosave·byline·edit·spellcheck·dateFormat·glyphFavorites·glyphKeymap이 결과 `next`에 누락 없이 담겨야 한다(상호 보존).
...
1. **등록/목록/영속까지만(effect 금지)**: 약물의 에디터 입력·약물바·키조합 인터셉트를 결선하지 마라(아래 DEFERRED EFFECTS). **Editor.jsx·WriterPage.jsx를 건드리지 마라.** 특히 WriterPage의 약물바 placeholder(`showGlyphBar` 등)를 손대지 마라(별개 — aux-tools 소관).
2. **순수 배열 연산**: 등록/삭제는 `[...arr, x]`/`filter`로 새 배열 생성(기존 배열 mutate 금지 — `push`/`splice`/직접 인덱스 할당 금지). 이유: React state 불변성·예측 가능한 리렌더.
...
5. **상호 보존**: 8개 카테고리(colors·autosave·byline·edit·spellcheck·dateFormat·glyphFavorites·glyphKeymap)를 함께 저장한다.
...
- **상호 보존**: 약물만 바꿔 '적용'해도 colors·autosave·byline·edit·spellcheck·dateFormat이 기존 값 그대로 유지된다.
...
1. 위 AC 커맨드를 실행한다.
2. 아키텍처 체크: ADR-003(editorPrefs는 view 모듈 — 서버 호출 없음) / DB 비파괴(무관, server/ 무변경) / 기존 탭 회귀 없음 / Editor.jsx·WriterPage.jsx 무변경 / 순수 배열 연산(mutate 없음).
3. `phases/16-editor-prefs-remaining/index.json`의 step 3을 갱신한다(성공 → `completed` + `summary` / 3회 실패 → `error` + `error_message` / 개입 필요 → `blocked` + `blocked_reason`).
4. 작업 종료 후 Slack harness 채널에 결과를 보고한다(오케스트레이터 소관).

## DEFERRED EFFECTS

이 step은 약물 등록/목록을 **저장만** 한다. 다음은 결선하지 않는다:
- 등록된 약물의 에디터 본문 입력(약물 입력 — news.md L180 도구>약물 입력)·약물바(glyph bar) 렌더·사용자 키조합 인터셉트(키 입력 → 약물 치환) → **aux-tools 후속 phase**(Editor 입력 경로·키 핸들러 변경 필요 — 이 phase 범위 밖).

## 금지사항

- **Editor.jsx·WriterPage.jsx를 수정하지 마라.** 이유: 약물 입력/약물바/키조합 인터셉트는 aux-tools 후속 phase이며, Editor 타이핑/IME/캐럿 불변식을 보호해야 한다.
- WriterPage의 약물바 placeholder(`showGlyphBar` 등)를 건드리지 마라(별개 — 이 step과 무관).
