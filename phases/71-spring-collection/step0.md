...
Spring 계약 하네스(`scripts/spring-contract.mjs`)의 프로파일 표에 **`host`·`spool`·`token` 축**을 세우고, 다섯 번째 프로파일 **`failclosed`**를 신설한다. 이 phase의 수집 2 라우트와 후속 배부 phase의 tick은 **서버 구성(바인드 주소·수집 토큰·스풀 루트)이 계약의 입력**이라, 하네스가 그 구성을 주입하지 못하면 계약 파일을 **돌릴 수단 자체가 없다**.

이 step의 산출물은 **`phases/72-spring-distribution`도 그대로 상속한다**(그 phase가 다시 설계하지 않는다) — `default.spool=true`가 여기서 정해지므로 배부 phase의 스풀 주입도 이 step이 미리 세워 둔다.
...
- `phases/71-spring-collection/index.json` — scope · baseline · order (c) · decisions **(1)(2)(3)(14)** · open_questions **(b)**
...
- **[함정 — 이 step의 핵심 실측] `files: []`로 프로파일을 올리면 그 프로파일은 '케이스 0건'이 아니라 '디렉토리 전건'이 된다.** `scripts/spring-contract.mjs` **382·418행**은 `profile.absFiles`가 비면 러너에 `--files`를 **아예 넘기지 않고**, `scripts/contract-run.mjs` **261~269행 `resolveCaseFiles`**가 그때 `contract/cases/<profile>/*.contract.js`를 **디렉토리 스캔**한다. `contract/cases/failclosed/collection-disabled.contract.js`는 **이미 존재**하므로, 수집 라우트가 없는 step0~4에서 Spring 패스는 그 케이스를 돌려 실패하고 Node 패스는 통과해 **`--parity`가 확정 red**가 된다. 같은 함정을 `spring-contract.mjs` **83~84행의 minimal 주석이 이미 경고**한다("files는 반드시 명시한다 — 비우면 러너가 디렉토리를 스캔해 … 설정 실수가 계약 실패로 위장된다"). 그래서 이 step은 `files: []`가 아니라 **명시 플래그**로 그 프로파일을 '기동 전용'으로 표시한다(작업 C).

## 작업

### A. Node 대조 리포트로 실측부터(decisions (14))
...
### C. `failclosed` 프로파일 신설 — **`bootOnly` 플래그로 올린다**

- `{ name:'failclosed', host:'0.0.0.0', spool:false, token:false, files:[], bootOnly:true, extraEnv:{} }`.
- **`bootOnly: true`의 의미와 `main()`의 처리(둘 다 이 step에서 구현한다)**:
  1. **`--profile` 미지정(다중 프로파일 실행)에서는 그 행을 실행 목록에서 제외**한다. 그 사실을 stdout 한 줄로 보고한다(예: `[failclosed] skipped — bootOnly(케이스 편입은 step5)`). 조용히 빠뜨리지 마라 — 사라진 프로파일은 '통과'로 오독된다.
  2. **`--profile failclosed`를 명시하면 실행하되, `--boot-check`가 없으면 `usageDie`로 거부**한다(메시지에 이유를 담는다: 케이스 파일이 비어 있어 러너가 디렉토리를 스캔하고 그 결과가 확정 red이므로).
  3. 따라서 `--parity`·`--dual-run`은 이 행을 **아예 만나지 않는다**(Node 대조 패스도 돌지 않는다).
- `files`가 비어 있는 것은 이 플래그와 **짝**이다. 주석에 "케이스 편입과 `bootOnly` 제거는 step5" · "`files`만 비우고 플래그를 빼면 러너 디렉토리 스캔으로 확정 red가 된다(위 배경 참조)"를 남긴다.
- `SCOPE_NAMES`·usage 문자열이 자동으로 5개가 되는지 확인한다(하드코딩된 프로파일 목록이 있으면 함께 갱신). **다만 usage의 "미지정=scope 표 전부"는 이제 거짓 안내다** — `failclosed`는 다중 프로파일 실행에서 제외되기 때문이다. usage 목록에 `failclosed=bootOnly(기동 전용 — `--profile failclosed --boot-check`로만 실행)` 주석을 함께 넣어라. step5가 플래그를 제거할 때 그 주석도 함께 지운다.
- **step5가 하는 일(여기서 하지 마라)**: `files`를 채우고 `bootOnly`를 **제거**한다 — 그때부터 다중 프로파일 실행·`--parity`가 이 프로파일을 정상 대상으로 본다.
...
4. **`bootOnly`가 아닌 프로파일의 `files`가 비어 있지 않은가** — 비어 있으면 `usageDie`(러너 디렉토리 스캔이 '설정 실수를 계약 실패로 위장'하는 경로를 원천 차단한다. 이 검사 하나가 이 step의 high 결함을 영구히 막는다).
...
- 1번: exit 0(jar 존재 확보).
- 2번: exit 0 · 로그에 `host=0.0.0.0` · health 왕복 성공 · `sessions ok roles=R,D,Z` · java 프로세스 잔존 0.
  **실패 서명 판별(open_questions (b))**: Windows 방화벽 인바운드 프롬프트가 뜬 채 응답이 없으면 `waitHealthy`가 **`--timeout`(기본 45000ms)까지 대기한 뒤 기동 실패**로 보고하고 진단에 java stdout/stderr가 붙는데 **자바는 정상 기동해 있다**(스택트레이스 없이 부팅 로그만 있다). 이 서명이면 코드 문제가 아니라 환경 문제이므로 **step을 `blocked`로 보고해 사용자 승인을 받는다** — 우회 바인딩(127.0.0.1로 낮추기 등)을 임의로 만들지 마라(fail-closed 축을 측정하는 의미가 사라진다).
- 3번: exit 0 · **4 프로파일** 실행(default·minimal·auth-negative·prod-cookie) + **`failclosed`는 `bootOnly`로 skip**되었다는 stdout 한 줄 · diffs **0** · 관측 수 **215 불변**(default 163 · minimal 45 · auth-negative 4 · prod-cookie 3). 관측 수가 움직였으면 env 주입이 서버 동작을 바꾼 것이므로 실패다.
- 4번: exit 0 · 4 프로파일 diffs 0(+ failclosed skip) · 프로파일마다 두 패스의 **pid·port·DATA_DIR·DIST_SPOOL_DIR이 서로 다른지** 로그로 확인.
- 5번 증분 = `scripts/spring-contract.mjs` · `phases/71-spring-collection/index.json`(step 상태) **뿐**.
...
4-b. **`bootOnly` 변이(원복) — 이 step의 핵심 실증**: `failclosed` 행에서 `bootOnly`를 빼고 `--parity`를 돌려 **그 프로파일이 디렉토리 스캔으로 `collection-disabled.contract.js`를 집어 확정 red가 되는지** 실측한다(실패 로그를 요약에 남긴다) → 원복 후 green 재확인. 그리고 `--profile failclosed`(`--boot-check` 없이)가 **usage 실패로 거부**되는지 확인한다.
5. **stdout/stderr·리포트 어디에도 토큰 값이 없는지** 확인한다(`--keep`으로 남긴 리포트를 문자열 검색).
6. 리포 `news.db` size·mtime, `uploads/` 항목 수가 실행 전후 무변인지 확인.
7. `phases/71-spring-collection/index.json`의 step0 상태를 갱신한다.
...
- **`files: []`만 두고 `bootOnly` 플래그를 생략하지 마라.** 이유: 러너가 `--files` 부재를 '디렉토리 전건'으로 해석해(`resolveCaseFiles`) `collection-disabled.contract.js`를 돌리고, 그 red가 '설정 실수'가 아니라 '계약 실패'로 보고된다 — `--parity`가 step0~4 내내 확정 red가 된다.
