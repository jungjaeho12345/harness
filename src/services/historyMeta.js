...
// ArticleHistory에는 제목·버전 저장 컬럼이 없고 상태는 전이 행에만 있다(스키마 변경 0 — DB 비파괴).
// 이 모듈은 이력 로그 + 편집 스냅샷(markupVersion)에서 표시용 title/version/status를 파생한다.
...
// snapshots: [{ id, markupVersion }] 배열 하나로 고정(undefined/빈 배열 허용).
...
//   title·status는 값이 없으면 ''(표시 폴백 '—'는 뷰 책임). markupVersion은 반환 행에 싣지 않는다.
export function decorateHistoryRows(rows, snapshots, { v1Body } = {}) {
  const input = Array.isArray(rows) ? rows : [];
  const snapMap = new Map(
    (Array.isArray(snapshots) ? snapshots : []).map((s) => [s.id, s.markupVersion]),
  );
  ...
  for (const r of asc) {
    if (hasSnap(r)) {
      ...
      title = snapshotTitle(snapMap.get(r.id));
    }
