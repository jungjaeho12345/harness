  ...
  async function run({ actorUserId = null } = {}) {
    ...
    for (const contents of candidates) {
      ...
      try {
        ...
        const due = dueKinds({ status: contents.status, contents, distributed: done, now: at });

        if (due.length === 0) {
          // 재정합(self-heal): 이력은 있는데 승격이 누락된 기사(이력 insert 실패·과거 데이터)가
          // 영원히 대기 상태로 남지 않게 한다. 바꿀 게 없으면 embargoStatusFor가 null을 주므로 쓰기 0건이다.
          if (contents.status === 'DES' || contents.status === 'EPS') {
            syncStatus(articleId, [], actorUserId, contents.status);
          }
          continue;
        }
        ...
        const status = syncStatus(articleId, okKinds, actorUserId, contents.status);
