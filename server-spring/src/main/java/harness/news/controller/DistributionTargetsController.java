...
public class DistributionTargetsController {
	...
	/** 수정 — Z 전용. present-only. 없는/비수치 id → 404 not-found(서비스가 NaN 수렴). */
	...
	public void update(@PathVariable String id, HttpServletRequest request, HttpServletResponse response) {
		DistributionTargetService.Result result =
				this.targets.update(tokenOf(request), id, this.json.readBody(request));
		if (!result.ok()) {
			deny(request, response, result.reason());
			return;
		}
	...
	/** 비활성(soft delete) — Z 전용. 본문 없음. 행을 지우지 않고 active='N'으로 내린다. */
	...
	public void deactivate(@PathVariable String id, HttpServletRequest request, HttpServletResponse response) {
		DistributionTargetService.Result result = this.targets.deactivate(tokenOf(request), id);
		if (!result.ok()) {
			deny(request, response, result.reason());
			return;
		}
	...
	/**
	 * 쿼리 파라미터 → 필터 맵(파라미터당 첫 값). {@code getParameterValues}로 읽는다 — {@code getParameter(}는
	 * {@code ?session=} 폴백 부활을 막는 정적 스캔({@code WebWiringTest})이 금지한 API다.
	 */
