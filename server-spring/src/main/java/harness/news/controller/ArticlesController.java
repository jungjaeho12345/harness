...
/**
 * 기사 단건 3라우트 — 생성 {@code POST /api/articles} · 조회 {@code GET /api/articles/:id} ·
 * 부분 수정 {@code PUT /api/articles/:id}. 리포 루트 {@code server/index.js}의 같은 3라우트와 1:1이다.
 *
 * <h2>컨트롤러가 하는 일은 넷뿐이다(ADR-006 · index.json decisions (13))</h2>
 * <ol>
 *   <li><b>신원 재도출</b> — 쿠키 우선 · {@code x-session-id} 폴백으로 토큰을 읽고 세션에서 신원을 얻는다.</li>
 *   <li><b>게이트 호출</b> — 생성은 역할 집합(R/D/Z), 수정은 잠금 보유자({@link EditLockService}).</li>
...
public class ArticlesController {
	...
	public ArticlesController(SessionGuard sessions, ArticleReadService reads, ArticleWriteService writes,
			EditLockService locks, JsonHttp json) {
	...
	public void update(@PathVariable String id, HttpServletRequest request, HttpServletResponse response) {
		...
		// 헤더 부재는 null 그대로다(빈 문자열 정규화 금지 — decisions (11)).
		EditLockService.Requester who = new EditLockService.Requester(actor.userId(), token,
				request.getHeader(EDIT_CLIENT_HEADER));
		EditLockService.LockResult holder = this.locks.assertHolder(id, who);
		if (!holder.ok()) {
