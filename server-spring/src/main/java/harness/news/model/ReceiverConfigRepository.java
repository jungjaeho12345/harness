...
public class ReceiverConfigRepository {
	 ...
	 * A의 되읽기 순서에서 A가 <b>B의 id</b>를 받는다 — 그 id로 삭제하면 <b>남의 설정 행</b>이 사라진다
	 * (행 삭제가 허용된 유일한 테이블이다). Node는 단일 스레드라 없는 결함이라 계약이 관측하지 않는다 —
	 * {@code ReceiverConfigRepositoryTest}의 동시 삽입 테스트가 유일 방어선이다.
	 *
