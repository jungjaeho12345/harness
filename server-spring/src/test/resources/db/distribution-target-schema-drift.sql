-- 테스트 전용 드리프트 픽스처(배부 수신처 축). 정본은 리포 루트의 `src/db/schema.js`이며 main 소스에는
-- 스키마 정의 SQL이 없다.
--
-- User·Article·Contents·ArticleHistory·ReceiverConfig는 정본과 같고 **DistributionTarget에서 2컬럼만
-- 빠져 있다**(spoolDir · updatedAt). 결함이 정확히 그 둘뿐이라, 넓어진 부팅 검증이 "무엇이 없는지"를
-- 다른 결함에 가려지지 않은 채(테이블 없음이 아니라 컬럼 단위로) 지목하는지 실증할 수 있다.
--
-- spoolDir가 없는 DB로 뜨면 배부 수신처의 스풀 폴더 지정이 통째로 깨지고, updatedAt이 없으면 감사
-- 타임스탬프가 실패한다 — 부팅에서 잡혀야 한다.
...
-- DistributionTarget — spoolDir·updatedAt 두 컬럼이 빠졌다(그 둘이 이 픽스처의 결함이다).
